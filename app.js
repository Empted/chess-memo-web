...
function draw(show=true) { board.innerHTML=''; for(let r=0;r<8;r++)for(let c=0;c<8;c++){const el=document.createElement('div');el.className=`square ${(r+c)%2?'dark':'light'}`;const p=pos[r][c];if(show&&p){el.textContent=glyph[p];el.classList.add(p===p.toUpperCase()?'white':'black');}if(c===0)el.insertAdjacentHTML('afterbegin',`<span class="rank">${8-r}</span>`);if(r===7)el.insertAdjacentHTML('beforeend',`<span class="file">${String.fromCharCode(97+c)}</span>`);board.append(el);}}
